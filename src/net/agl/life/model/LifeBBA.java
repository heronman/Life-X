...
public class LifeBBA implements Life {
	...
	public boolean isValid(int x, int y) {
		return (cells != null && y >= 0 && y < cells.length && x >= 0 && x < cells[0].length);
	}
	...
	public void setSize(int width, int height) {
		...
		if (cells != null && cells.length == height && cells[0].length == width)
			return;
		boolean[][] newcells = new boolean[height][width];
		if (cells != null)
			alive = copy(cells, newcells);
		cells = newcells;
	}
	...
	private int countNeighbors(int x, int y) {
		...
		for (int _y = y - 1; _y - y < 2; _y++) {
			if (_y < 0 || _y >= cells.length)
				continue;
			for (int _x = x - 1; _x - x < 2; _x++) {
				if (_x < 0 || _x >= cells[_y].length || _y == y && _x == x)
					continue;
				if (cells[_y][_x])
					n++;
			}
